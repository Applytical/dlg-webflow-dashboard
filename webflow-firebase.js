...
firebase.analytics && firebase.analytics();
{
  ...
  firebase.auth().onAuthStateChanged(function (authUser) {
    ...
    if (user) {
      ...
      //const subscriptionsTitle = document.getElementById('subscriptionsTitle').style.display = "none";
      ...
      if (!customerName) {
        ...
        //userDisplayName.forEach(function (el) { el.innerText = sessionStorage.getItem("Name"); });
      }

      function getCustomer() {
        axios.post(`${url}/webflow/customers`, {
          email: user.email
        })
          .then((response) => {

            const customerName = sessionStorage.setItem("Name", response.data.customer.firstName + " " + response.data.customer.lastName);
            user.updateProfile({
              displayName: response.data.customer.firstName + " " + response.data.customer.lastName,
            }).then(() => {
              // Update successful
              // ...
            }).catch((error) => {
      ...
      if (window.location.href == `${websiteUrl}/account/subscriptions`) {
        axios.post(`${url}/webflow/subscriptions/all`, {
          email: user.email
        })
          .then((response) => {
            getCustomerSubscriptions(response);
            sessionStorage.setItem("Subscriptions", response.data.subscriptions);
          });

        function getCustomerSubscriptions(response) {
  ...
  var actionCodeSettings = {
    ...
    url: `${websiteUrl}/account/subscriptions`,
    // This must be true.
  ...
  var emailLinkSuccess = document.querySelectorAll('[data-link-success]');
  var hideSubmit = document.querySelectorAll('[data-link-btn]');
  ...
  emailLink.forEach(function (el) {
    ...
    el.addEventListener('submit', function (e) {
      ...
      firebase.auth().sendSignInLinkToEmail(loginEmail.value, actionCodeSettings).then(() => {
        ...
        hideSubmit.forEach(function (el) { el.style.display = 'none'; });
        emailLinkSuccess.forEach(function (el) {
          el.style.display = 'block';
        });
  ...
  phoneLink.forEach(function (el) {
    var phoneNumber = el.querySelector('[data-form-phone]');
    ...
    el.addEventListener('submit', function (e) {
      ...
      firebase.auth().signInWithPhoneNumber(phoneNumber.value, appVerifier)
        .then((confirmationResult) => {
          ...
          let code = window.prompt("Please enter your SMS Code");
          let text;

          if (code == null || code == "") {
            text = "User cancelled the prompt.";
          } else {
            confirmationResult.confirm(code).then((authUser) => {
